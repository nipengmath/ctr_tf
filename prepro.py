...
class CacheData(object):
    def _load_batch_data(self, file, config):
        """
    ...
    def _convert_data(self, labels, features, config):
        dim = config.FEATURE_COUNT
    ...
    def write_tfrecord(self, infile, outfile, config):
        sample_num = 0
        FEATURE_COUNT = config.FEATURE_COUNT
        writer = tf.python_io.TFRecordWriter(outfile)
        feature_cnt = defaultdict(lambda: 0)
        for labels, features, in self._load_batch_data(infile, config):
            sample_num += len(labels)
            print(infile, sample_num)
            input_in_sp = self._convert_data(labels, features, config)
        ...
        with open(config.feature_count_file, 'w') as f:
...
def prepro(config):
    ...
    if config.train_file:
        train_num = cache.write_tfrecord(
            config.train_file, config.train_record_file, config)
        save(config.train_meta, {"total": train_num}, message="train meta")

    if config.dev_file:
        dev_num = cache.write_tfrecord(
            config.dev_file, config.dev_record_file, config)
        save(config.dev_meta, {"total": dev_num}, message="dev meta")

    if config.test_file:
        test_num = cache.write_tfrecord(
            config.test_file, config.test_record_file, config)
        save(config.test_meta, {"total": test_num}, message="test meta")
